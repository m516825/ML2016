...
def feature_scaling(x_dat, train_len):
	...
	size = len(x_dat[0])
	mean = np.sum(np.array(x_dat), axis=0) / train_len
	...
	return x_dat, mean, driva

def test_scaling(x_dat, mean, driva):
...
def dump_model(w, b, mean, driva, model_name):
...
def main():
	...
	if args.type == 1:
		...
		train_x, mean, driva = feature_scaling(train_x, len(train_x))

		w, b, layer = logistic_regression(args, train_x, train_y)

		dump_model(w, b, mean, driva, args.model)

	else:

		test_x = load_test(args)

		w, b, mean, driva, layer = load_model(args.model)

		test_x = test_scaling(test_x, mean, driva)
