...
def count_ein(w, b, x_dat, y_dat, layer):
	...
	for i in range(size):
		ans = np.array([1.,0.]) if a[0, i] > a[1, i] else np.array([0.,1.])
		if ans != y_dat[i]:
			error += 1.
...
def back_prop(param_w, param_b, param_a, param_z, layer, y_dat):
	...
	for l in range(layer-1, -1, -1):
		if l == layer-1:
			theta[l] = (1 - sigmoid(param_z[l])) * sigmoid(param_z[l]) * (- np.log(param_a[l+1]) + np.log(1. - param_a[l+1]))#(param_a[l+1] - y_dat)
		else:
...
def logistic_regression(args, train_x, train_y):
	...
	# train_x, train_y = fixed_feature(train_x, train_y)
	batch_x, batch_y, batch_number = make_batch(train_x, train_y, 5)
