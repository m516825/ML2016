...
def logistic_regression(args, train_x, train_y):
	...
	NN = [f_size, 50, 20, 3, 20, 50, f_size]
	layer = len(NN)-1
	param_w = []
	param_b = []
	param_grad_w = []
	...
	for iters in range(args.iteration):
		...
		for i, dat in enumerate(batch_x):
			...
			_w, _b = back_prop(param_w, param_b, param_a, param_z, layer, batch_x[i]) 
	...
	for i, dat in enumerate(z.T):
		lis.append((dat[0], dat[1], dat[2], train_y[i]))
		# f.write(str(dat[0])+','+str(dat[1])+','+str(y_dat[i][0])+'\n')
	lis = sorted(lis, key=lambda (a,b,c,d):c)
	for i in lis:
		if i[3] == 0:
